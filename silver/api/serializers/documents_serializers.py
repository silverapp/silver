...
class DocumentSerializer(serializers.HyperlinkedModelSerializer):
    ...
    def get_transactions(self, document):
        ...
        if document.kind == 'invoice':
            transactions = Transaction.objects.filter(invoice_id=document.id).select_related('payment_method')
        elif document.kind == 'proforma':
            transactions = Transaction.objects.filter(proforma_id=document.id).select_related('payment_method')
