    ...
    def should_be_billed(self, date):
        ...
        if self.is_billed_first_time:
            ...
            if (self.trial_end and
                ...
                # The trial will end this month or it has already ended last
                ...
                # Get the end of the month in which the trial ends
                interval_end = list(
                    rrule.rrule(
                        rrule.MONTHLY,
                        count=1,
                ...
                # The most common case when the subscription is already past
                # of the trial period + other edge cases
                interval_end = self.bucket_end_date(
    ...
    def add_total_value_to_document(self, billing_date, invoice=None,
        ...
        if self.is_billed_first_time:
            if not self.trial_end:  # has no trial
                if billing_date.month in [self.start_date.month,
                    ...
                    if (billing_date.month == self.start_date.month and
                        ...
                        # of the customer => add the mfs too as the customer
                        # is probably leaving
                ...
                if billing_date.month == self.start_date.month + 1:
                    ...
                    # system did not work properly or the customer has
                    # consolidated billing => add the add the consumed mfs during
                    # the first bucket after the trial and if the subscription
                    # was not canceled => add the plan's for the next month

                    # Add the consumed mfs from the first bucket after trial
            ...
            if (self.trial_end and
                ...
                self._add_trial_value(start_date=start_date, end_date=end_date,
                                      invoice=invoice, proforma=proforma)
                ...
                if billing_date.month == first_day_after_trial.month + 1:
                    ...
                    # trial_end -> last_month_end=> add the consumed mfs for
                    # that period.
                    # Note: the start_date and end_date that were previously
                ...
                # sometimes at the beginning of last month.
